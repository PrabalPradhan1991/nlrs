...
class ServiceController extends Controller
{
    ...
    public function postCreateSideImage() {
        $input = request()->all();

        if(request()->hasFile('asset')) {
            request()->file('asset')->store('service-side-image');
            $filename = request()->file('asset')->hashName();

            (new \App\HelperModel)->storeHelperValue('service_side_image', $filename);
            
        }

        session()->flash('success-msg', 'Image successfully set');
