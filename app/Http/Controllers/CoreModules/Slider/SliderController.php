...
class SliderController extends Controller
{
    ...
    public function postSliderBackgroundImage() {
        $input = request()->all();

        if(request()->hasFile('asset')) {
            request()->file('asset')->store('slider-background');
            $filename = request()->file('asset')->hashName();

            (new \App\HelperModel)->storeHelperValue('slider_background_image', $filename);
            
        }

        session()->flash('success-msg', 'Background Image successfully set');
